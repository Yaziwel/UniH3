...
from model.UniH3_3D import UniH3
from loss.losses import CharbonnierLoss
from evaluation.evaluation_metric import compute_measure
from data.common import transformData, dataIO, restore_with_patches
...
def build_train_sampler(modality_list, src_root, batch_size, save_root, sampling='uniform'): 
    ...
    if sampling=='uniform': 
        for modality in modality_list:
            dataset = MedIRData(root_dir=src_root, modality_list = [modality], patch_size=64, preprocess=True, flag='train_patch', save_root=save_root, use_num=-1) 
            print("{}: {}".format(modality, dataset.length))
            dataloader_list.append(DataLoader(dataset, batch_size=batch_size, shuffle=True, drop_last=True, num_workers=2, collate_fn=custom_collate_fn)) 
        
    elif sampling=='random':
        dataset = MedIRData(root_dir=src_root, modality_list = modality_list, patch_size=64, preprocess=True, flag='train_patch', save_root=save_root, use_num=-1) 
...
src_root = "/home/data/zhiwen/dataset/All-in-One-7-Task/3D/" ### Path to place data
# modality_list = ["PET", "CT", "MRI", "OCT", "Pathology", "Ultrasound", "X-ray"] 
modality_list = ["MRI", "CT", "PET"] 
...
train_sampler = build_train_sampler(modality_list, src_root, batch_size, save_root, sampling='uniform') 
...
for name, param in Generator.named_parameters():
    if param.ndim == 5 and param.shape[-3:] != (1, 1, 1):
        group_kxkxk.append((param))
    else:
        group_other.append((param)) 
print("length", len(group_kxkxk))
param_groups = [
    dict(params=group_kxkxk, use_muon=True,
...
for iteration in list(range(1, int(total_iteration)+1)): 
    ...
    lq_img = torch.cat(lq_img, dim=0)
    hq_img = torch.cat(hq_img, dim=0) 
    ...
    if iteration % val_iteration == 0: 
        ...
        for counter, data in enumerate(tqdm(valid_loader)):
            ...
            with torch.no_grad():
                ...
                gen_img = restore_with_patches(model_ema, lq_img).detach().cpu() 
