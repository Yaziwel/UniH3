...
class PixelShuffle3D(nn.Module):
    """Rearrange channels into depth, height and width."""
    ...
    def forward(self, x):
        ...
        b, c, d, h, w = x.shape
        if c % (r ** 3) != 0:
            raise ValueError(f"Channels ({c}) must be divisible by scale^3 ({r ** 3}).")
        out_c = c // (r ** 3)
...
class PixelUnshuffle3D(nn.Module):
    """Rearrange depth, height and width into channels."""
    ...
    def forward(self, x):
        ...
        if d % r != 0 or h % r != 0 or w % r != 0:
            raise ValueError(
                f"D/H/W must be divisible by scale ({r}); got D={d}, H={h}, W={w}."
            )
        out_d, out_h, out_w = d // r, h // r, w // r
        x = x.reshape(b, c, out_d, r, out_h, r, out_w, r)
        x = x.permute(0, 1, 3, 5, 7, 2, 4, 6).contiguous()
        return x.reshape(b, c * (r ** 3), out_d, out_h, out_w)
...
class ImageUnShuffle(nn.Module):
    def __init__(self, in_feat, n_feat, scale):
        ...
        self.body = nn.Sequential(PixelUnshuffle3D(scale),
                                  nn.Conv3d(in_feat*scale**3, n_feat, kernel_size=3, stride=1, padding=1, bias=False)
                                  )
...
class H2M(nn.Module):
    def __init__(self, task_list, num_prompt=4096, dim=384, heads=8, momentum=0.99):
        ...
        self.q_project = nn.Conv3d(dim, dim, kernel_size=1, bias=False)
        self.k_project = nn.Linear(dim, dim, bias=False)
        self.v_project = nn.Linear(dim, dim, bias=False)

        self.q_dwconv = nn.Conv3d(dim, dim, kernel_size=3, stride=1, padding=1, groups=dim, bias=False)
    ...
    def forward(self, x_lq, x_hq=None, task_list=None):
        ...
        x_lq: [B, C, D, H, W]
        x_hq: [B, C, D, H, W] or None
        task_list: list[str] or tensor([task_id_0, ...])
        """
        B, C, D, H, W = x_lq.shape
...
class IR(nn.Module):
    def __init__(self, 
        ...
        self.reduce_refinement = nn.Conv3d(int(dim*3), dim, kernel_size=1, bias=bias)
    ...
    def forward(self, inp_img, label_img=None, inp_task_list=None): 
        if inp_img.ndim != 5:
            raise ValueError(f"inp_img must have shape [B, C, D, H, W], got {tuple(inp_img.shape)}.")
        if any(size % 8 != 0 for size in inp_img.shape[-3:]):
            raise ValueError(
                f"Input D/H/W must each be divisible by 8, got {tuple(inp_img.shape[-3:])}."
            )
        if label_img is not None and label_img.shape != inp_img.shape:
            raise ValueError(
                f"label_img must match inp_img shape; got {tuple(label_img.shape)} and {tuple(inp_img.shape)}."
            )
...
class UW_Estimator(nn.Module):
    def __init__(self, in_dim=48, out_dim=1, hidden_dim=48, task_list=[], mode="log"):
        ...
        self.un_conv = nn.Sequential(
            nn.Conv3d(in_dim, hidden_dim, 3, 1, 1, bias=True), 
            nn.GELU(),
            nn.Conv3d(hidden_dim, hidden_dim, 3, 1, 1, bias=True), 
            nn.GELU(),
            nn.Conv3d(hidden_dim, out_dim, 3, 1, 1, bias=True),
        ) 
    def forward(self, condition, inp_task_list):
        # First linear transformation 
        uncertainty_img = self.un_conv(condition) 
        uncertainty_weight = uncertainty_img.mean(dim=(1,2,3,4)) 
