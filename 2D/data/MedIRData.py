...
import os
import numpy as np
from .common import dataIO, transformData, ShapeAligner
import pdb 
import random
io=dataIO() 
...
class MedIRData(Dataset):
    def __init__(self, root_dir, modality_list = ["PET"], patch_size=128, preprocess=True, flag='train', save_root=-1, use_num=-1):
    ...
    def __getitem__(self, idx): 
        ...
        if modality in ['PET', 'CT', 'MRI']:
            ...
            spacing = np.array([1, 1, 1], dtype=float) 
        ...
        data_dict = {
            ...
            'round': False if modality=='PET' else True
            } 
...
if __name__ == "__main__": 
    ...
    data_root = "/home/data/zhiwen/dataset/All-in-One-7-Task/2D/" 
    # modality_list = ["PET", "CT", "MRI", "OCT", "Pathology", "Ultrasound", "X-ray"] 
    modality_list = ["PET"]
    train_loader_list = []

    save_root = "/home/data/zhiwen/experiment/All-in-One-7-Task/2D/data/"

    
    dataset = {
        'train': MedIRData(root_dir=data_root, modality_list = modality_list, patch_size=128, preprocess=True, flag='train', save_root=save_root), 
    ...
    for counter, data in enumerate(tqdm(train_loader)): 
        ...
        # check_lq = lq_img[0].clone().numpy()*20.0
        ...
        lq_img = shape_aligner.unpad_images(lq_img) 
        hq_img = shape_aligner.unpad_images(hq_img)
        
        # pdb.set_trace()

        for i in range(len(lq_img)):
            data[i]['save_data']=lq_img[i] 
            data[i]['save_path']=data[i]['save_path'].replace(".nii", "_LQ.nii")
        data = transform.postprocess(data, process_hq=False) 
        [io.save(d) for d in data] 
        
        # pdb.set_trace()
        
        # for i in range(len(hq_img)):
        #     data[i]['save_data']=hq_img[i] 
        #     data[i]['save_path']=data[i]['save_path'].replace("_LQ.nii", "_HQ.nii") 
        # # pdb.set_trace()
        # data = transform.postprocess(data, process_hq=False) 
        # [io.save(d) for d in data]    
        ...
        # check_data = data[0] 
        # src_path = check_data['lq_path'] 
        # target_path = check_data['save_path'] 

        # img1 = io.load(src_path)['data'] 
        # img2 = io.load(target_path)['data'] 
